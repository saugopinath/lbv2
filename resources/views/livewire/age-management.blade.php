<div class="bg-white shadow rounded-xl p-6 border border-gray-200">
    <h2 class="text-xl font-bold text-indigo-700 mb-6 border-b pb-2">Age Management Configuration</h2>
    <form wire:submit.prevent="save" class="space-y-6">
        ...
        @if ($isspecial === 'yes')
            <div class="space-y-4 pt-4">
                <div class="flex justify-between items-center">
                    ...
                    <button type="button" wire:click="addSpecialCase" class="bg-indigo-600 text-white px-4 py-2 rounded shadow hover:bg-indigo-700 text-sm">
                        + Add More
                    </button>
                </div>
                @foreach ($selectedSpecialCases as $index => $case)
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end bg-indigo-50 p-4 rounded-lg border border-indigo-100 shadow-sm">
                        <div class="md:col-span-1">
                            <x-form.select label="Category" name="selectedSpecialCases.{{ $index }}.case_id" wire:model.live="selectedSpecialCases.{{ $index }}.case_id">
                        ...
                        <div>
                            <x-form.input label="Min Age" name="selectedSpecialCases.{{ $index }}.min" wire:model.defer="selectedSpecialCases.{{ $index }}.min" 
                                x-on:input="$el.value = $el.value.replace(/[^0-9]/g, '').slice(0,3); $wire.set('selectedSpecialCases.{{ $index }}.min', $el.value);" />
                        </div>
                        <div>
                            <x-form.input label="Max Age" name="selectedSpecialCases.{{ $index }}.max" wire:model.defer="selectedSpecialCases.{{ $index }}.max" 
                                x-on:input="$el.value = $el.value.replace(/[^0-9]/g, '').slice(0,3); $wire.set('selectedSpecialCases.{{ $index }}.max', $el.value);" />
                        </div>
                        <div class="flex justify-center pb-2">
                            <button type="button" wire:click="removeSpecialCase({{ $index }})" class="text-red-500 hover:text-red-700 font-bold">
                                Remove
                            </button>
                        </div>
                    </div>
        ...
        @endif
        <div class="border-t">
            <x-button.loading-button action="save" text="Save Changes" class="bg-indigo-700 text-white py-3 rounded-xl font-bold mt-6" />
        </div>
    </form>
</div>