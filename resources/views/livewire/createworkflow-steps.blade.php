        ...
        <div class="grid gap-6 md:grid-cols-2 mb-2 pl-4 pr-4">
            <div>
                <x-form.input name="noofSteps" label="No of Steps" wire:model.live="noofSteps"
                    x-on:input="$event.target.value = $event.target.value.replace(/[^0-9]/g, '').slice(0,2)" required />
            </div>
        </div>
        @if ($noofSteps > 0)
            <div class="mt-4">
                    ...
                    <div class="grid gap-6 md:grid-cols-2 mb-2 pl-4 pr-4">
                        <div>
                            <x-form.input id="labels.{{ $index }}" name="labels.{{ $index }}"
                                label="Label Name {{ $index + 1 }}" placeholder="Label Name {{ $index + 1 }}"
                                wire:model="labels.{{ $index }}" required />
                        </div>
                    </div>
                @endforeach
                @if ($this->already)
                    <p class="text-sm font-semibold text-green-700 bg-green-100 px-3 py-1 rounded-lg inline-block">
                        Already Done
                    </p>
                @else
                    <div class="mt-6 pl-4">
