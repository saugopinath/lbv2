<div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">HOF EPIC/Voter
                    No. @if($this->getAgeFromDob($formData['hof_dob'] ?? '') >= 18) * @endif <br><span class="text-xs text-gray-500 font-normal">ভোটার কার্ড
                        নম্বর</span></label>
                <div x-data="{
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Assembly Constituency No. @if($this->getAgeFromDob($formData['hof_dob'] ?? '') >= 18) * @endif <br><span
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Part No. of Electoral Roll @if($this->getAgeFromDob($formData['hof_dob'] ?? '') >= 18) * @endif <br><span
        ...
        <div
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Member
                    EPIC/Voter No. @if($this->getAgeFromDob($members[$index]['dob'] ?? '') >= 18) * @endif <br><span class="text-xs text-gray-500 font-normal">ভোটার কার্ড
                        নম্বর</span></label>
                <div x-data="{
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Assembly Constituency No. @if($this->getAgeFromDob($members[$index]['dob'] ?? '') >= 18) * @endif
                    <br><span class="text-xs text-gray-500 font-normal">বিধানসভা নং</span></label>
                <select wire:model.live="members.{{ $index }}.assembly_constituency"
                    class="w-full border border-gray-300 rounded p-2 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    <option value="">-- Select AC --</option>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Part No. of Electoral Roll @if($this->getAgeFromDob($members[$index]['dob'] ?? '') >= 18) * @endif
                    <br><span class="text-xs text-gray-500 font-normal">পার্ট নং</span></label>
                <input type="text" wire:model.live="members.{{ $index }}.part_no"
                    class="w-full border border-gray-300 rounded p-2 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                @error("members.{$index}.part_no")
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-lg p-5 mt-4 text-center text-amber-900 shadow-sm">
        ...
        <p class="text-[10px] text-amber-700 mt-1"> sishu sadoshoder jonyo bank account abong voter biboronir
            proyojon nei.</p>
    </div>
    @endif
    @endif
