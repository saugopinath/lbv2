<div>
    <div x-data="{
        ...
        <div class="flex justify-center space-x-3" x-show="selectedOption && fields[selectedOption]" x-transition>
            <button type="button" wire:click="search" x-on:click="$dispatch('showLoader')"
            ...
            @if ($isReset)
            <button type="button" wire:click="resetSearch" x-on:click="$dispatch('showLoader')"
                class="px-8 py-2.5 bg-gray-500 backdrop-blur-md border border-white/30 text-white font-medium rounded-lg hover:bg-gray-600 focus:ring-2 focus:ring-gray-600 transition-all duration-300 shadow-lg flex items-center justify-center space-x-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
