<x-layouts.app>
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-xl p-4 mb-4">
        <div class="flex items-center space-x-3">
            <h1 class="text-xl font-bold text-indigo-800 dark:text-white">
                Update Mobile Details For Approved Beneficiary
            </h1>
            <span
    ...
    @endif
    <div x-data="{ openSection: 'update-mobile' }"
        class="space-y-4 bg-white dark:bg-gray-800 shadow-md rounded-xl p-4 mb-6">

        <!-- Accordion Header -->
        <div class="rounded overflow-hidden">
            <button @click="openSection = openSection === 'update-mobile' ? '' : 'update-mobile'"
                ...
                <!-- Plus Icon -->
                <svg x-show="openSection !== 'update-mobile'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"
            ...
            <!-- Accordion Content -->
            <div x-show="openSection === 'update-mobile'" x-transition.opacity.duration.400ms
                class="transition duration-500 p-6 bg-green-50 shadow border-l-4 border-blue-500 rounded-b-md space-y-4 mt-2">
                <form method="POST" action="{{ route('update-mobile') }}" x-data @submit.prevent="
        if ($refs.mobile.value.trim() === '' || $refs.revert_reason_remarks.value.trim() === '') {
            alert('Please fill all required fields before submitting.');
            return;
        }
        if (confirm('Are you sure you want to update this mobile number?')) {
            $el.submit();
        }
    ">
                    @csrf
                    ...
                    <div class="grid md:grid-cols-3 gap-6">
                        ...
                        <div class="col-span-1">
                            <x-form.input id="mobile" name="mobile" value="{{ old('mobile', $mobileNumber) }}"
                                label="Mobile Number" required x-ref="mobile" x-on:input="
