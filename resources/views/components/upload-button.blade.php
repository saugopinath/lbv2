        ...
        if (file_exists($jsonPath)) {
            ...
            foreach ($docsArray as $doc) {
                if (isset($doc['doc_type_id']) && $doc['doc_type_id'] == $docId) {
                    if (empty($docName)) $docName = $doc['doc_name'];
                    if (empty($buttonText) && isset($doc['button_text'])) $buttonText = $doc['button_text'];
                    break;
                }
            }
        }
...
<div class="mt-2 flex items-center justify-between" x-data>
    <button type="button" @click="$dispatch('open-upload-modal', {docId: {{ $docId }}, docName: '{{ addslashes($docName) }}'})"
    ...
    <div x-show="$wire.uploadedDocuments[{{ $docId }}]" class="flex items-center gap-2">
        ...
        <button type="button" wire:click="downloadDocument($wire.uploadedDocuments[{{ $docId }}])" class="text-blue-600 underline text-xs font-medium">View</button>
    </div>
</div>
