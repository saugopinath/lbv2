...
@endphp
<aside :class="sidebar ? 'w-60' : 'w-16'" class="transition-all duration-300 bg-gradient-to-b from-[#b34700] via-[#e06b00] to-[#f5a623] shadow-2xl flex flex-col h-screen border-r border-orange-900/30" x-data="{ activeMenu: '{{ $activeMenu }}' }">
    <!-- Logo -->
    <div class="flex flex-col items-center border-b border-orange-900/30 bg-white py-2 {{ config('jblbConf.das_logo_class') }}">
        <img alt="Annapurna Yojana" class="{{ config('jblbConf.logo_das_width') }}" src="{{ asset('images/' . config('jblbConf.das_logo')) }}" />
    ...
    <nav class="flex-1 overflow-y-auto mt-2 space-y-1 text-sm">
        ...
        @foreach ($menuItems as $item)
            @php
                $canShow = !isset($item['permission']) || \App\Helpers\WorkFlowPermissionHelper::{$item['permission']}();
            ...
            @if ($canShow)
                <div>
                    @if ($isGroup)
                        <button @click="activeMenu === '{{ $item['key'] }}' ? activeMenu = null : activeMenu = '{{ $item['key'] }}'" class="flex items-center w-full px-4 py-2.5 text-left transition-all duration-200 hover:bg-black/15 text-yellow-50 hover:text-white rounded-lg {{ $isGroupActive ? 'bg-black/20 border-l-4 border-yellow-300 text-white font-semibold' : '' }}">
                            <svg class="w-5 h-5 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                {!! $item['icon'] !!}
                            </svg>
                            <span class="mr-2 truncate" x-show="sidebar">{{ $item['label'] }}</span>
                            <svg :class="activeMenu === '{{ $item['key'] }}' ? 'rotate-180' : ''" aria-hidden="true" class="mr-2 w-3 h-3 transition-transform duration-200" fill="none" viewBox="0 0 10 6" xmlns="http://www.w3.org/2000/svg">
                                <path d="m1 1 4 4 4-4" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" stroke="currentColor" />
                            </svg>
                        </button>

                        <div class="pl-4" x-collapse x-show="activeMenu === '{{ $item['key'] }}'" x-transition>
                            <ul>
                                @foreach ($item['children'] as $child)
                                    @php
                                        $canShowChild = !isset($child['permission']) || \App\Helpers\WorkFlowPermissionHelper::{$child['permission']}();
                                        $childUrl = isset($child['params']) ? route($child['route'], $child['params']) : route($child['route']);
                                        $isChildActive = request()->fullUrl() == $childUrl;
                                    @endphp
                                    @if ($canShowChild)
                                        <li>
                                            <a class="flex items-center px-3 py-1.5 text-left text-yellow-100 rounded-md transition-all duration-200 hover:bg-black/15 hover:text-white {{ $isChildActive ? 'bg-black/20 text-yellow-200 font-semibold border-l-2 border-yellow-300' : '' }}" href="{{ $childUrl }}">
                                                <svg class="w-5 h-5 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    {!! $icons['child'] !!}
                                                </svg>
                                                <span class="truncate" x-show="sidebar">{{ $child['label'] }}</span>
                                            </a>
                                        </li>
                                    @endif
                                @endforeach
                    ...
                    @else
                        <a class="flex items-center w-full px-4 py-2.5 text-left transition-all duration-200 hover:bg-black/15 text-yellow-50 hover:text-white rounded-lg {{ $isActive ? 'bg-black/20 border-l-4 border-yellow-300 text-white font-semibold' : '' }}" href="{{ route($item['route']) }}">
                            <svg class="w-5 h-5 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                {!! $item['icon'] !!}
                            </svg>
                            <span class="mr-2 truncate" x-show="sidebar">{{ $item['label'] }}</span>
