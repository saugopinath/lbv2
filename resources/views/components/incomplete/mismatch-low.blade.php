<div x-data="{ selected: '' }" class="mt-4">
    ...
    <div class="flex gap-6 mt-6 pl-4 pr-4">
        ...
        <label class="flex items-center space-x-2">
            <input type="radio" class="form-radio text-blue-600"
                wire:model="formData.bank_action.{{ $item->id }}" value="3" x-model="selected" />
            <span>Reject</span>
        </label>
    </div>

    {{-- CHANGE --}}
    <div x-show="selected == 3" class="grid gap-6 mb-4 md:grid-cols-2 pl-4 pr-4">
        <div>
            <x-form.textarea id="reject_reason_remarks" name="reject_reason_remarks" label="Remarks" required />
        </div>
    </div>
</div>
