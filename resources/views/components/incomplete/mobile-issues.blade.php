{{--  @if (!empty($mobileIssues))
    <div class="p-4 mb-4 border rounded-lg bg-gray-50 shadow-sm">
        <h2 class="font-semibold text-lg text-blue-700 mb-2">Mobile Related Issues</h2>
...
{{--  @if (!empty($mobileIssues))
    <div class="p-4 mb-4 border rounded-lg bg-gray-50 shadow-sm">
        ...
        <p class="text-sm text-gray-600">Old Mobile: {{ $mobileIssues[0]->old_value ?? 'N/A' }}</p>
        <x-form.input
            id="mobile_{{ $mobileIssues[0]->id }}"
            name="mobile[{{ $mobileIssues[0]->id }}]"
            label="New Mobile Number"
            required
            wire:model="formData.new_mobile.{{ $mobileIssues[0]->id }}"
