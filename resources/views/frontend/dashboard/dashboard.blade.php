...
@section('content')
    ...
    <!-- Charts Section -->
    @if(config('jblbConf.is_jb') == 1)
        <!-- ============================================== -->
        <!-- JAI BANGLA (Multi-Scheme) LAYOUT               -->
        <!-- ============================================== -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            ...
            <div class="chart-container">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">Monthly DBT Payment Statistics</h3>
                        <p class="text-sm text-gray-500 mt-1">Year {{ $cur_fin_year }} overview</p>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-gray-500">
                        <i class="fas fa-info-circle"></i> Consolidated Payments
                    </div>
                </div>
            ...
            <div class="chart-container">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">Age breakdown</p>
                    </div>
            ...
            <div class="chart-container">
                <div class="chart-header">
                    <h2 class="chart-title"><i class="fas fa-rings mr-2 text-pink-500"></i>Marital Status</h2>
                </div>
                <div id="maritalStatusChart" class="chart-body h-[300px]"></div>
            </div>
            ...
            <div class="chart-container">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">Caste Distribution</h3>
                        <p class="text-sm text-gray-500 mt-1">Beneficiary breakdown</p>
                    </div>
                    <span class="text-sm text-gray-500"><i class="fas fa-users"></i></span>
                </div>
        ...
        <div class="grid grid-cols-1 gap-6 mt-6">
            ...
            <div class="chart-container">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">Daily Applications (Last 30 Days)</h3>
                        <p class="text-sm text-gray-500 mt-1">Applications received over the last 30 days</p>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-gray-500">
        ...
        <div class="bg-white rounded-xl shadow mt-6">
            <div class="flex items-center justify-between px-6 py-4 border-b">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800">Scheme-wise Consolidated Status</h3>
                    <p class="text-xs text-gray-500 mt-1">Consolidated workflow status across schemes</p>
                </div>
    ...
    @else
        <!-- ============================================== -->
        <!-- LAKSHMIR BHANDAR (Single Scheme) LAYOUT        -->
        <!-- ============================================== -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            <!-- Daily Applications (Takes full width on top) -->
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
            ...
            <div class="chart-container">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">Age Cohort</h3>
                    </div>
                </div>
                <div id="categoryChart" style="height: 350px;"></div>
            </div>
            ...
            <div class="chart-container">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">Gender</h3>
                    </div>
                </div>
            ...
            <div class="chart-container">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">Caste</h3>
                    </div>
                </div>
                <div id="casteChart" style="height: 350px;"></div>
            </div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const refreshBtn = document.getElementById('refreshSchemeStatus');
            if (refreshBtn) {
            ...
            if (document.getElementById('schemeStatusTable')) {
                loadSchemeStatusTable();
            }
    ...
    <script>
        function waitForHighcharts(callback) {
            if (typeof window.Highcharts !== 'undefined') {
                ...
                setTimeout(function () { waitForHighcharts(callback); }, 50);
            }
        }

        waitForHighcharts(function () {

            // Disable accessibility warning globally
            ...
            function loadDistrictWiseBeneficiaries() {
                fetch("{{ route('dashboard.districtWiseBeneficiaries') }}", {
                    ...
                    .then(res => res.json())
                    .then(function (response) {
                        // Guard: no data returned (e.g. approveRoles empty)
                        if (!response.categories || response.categories.length === 0) {
                            const el = document.getElementById('districtChart');
                            if (el) el.innerHTML = '<p class="text-center text-gray-400 pt-16">No district data available.</p>';
                            return;
                        }
                    ...
                    .catch(function (err) {
                        console.error('Failed to load district-wise beneficiary data', err);
                    });
            }
    ...
    <script>
        waitForHighcharts(function () {
            ...
            fetch("{{ route('dashboard.fy.consolidated') }}?fin_year={{ $cur_fin_year }}", {
                ...
                .catch(function () {
                    console.error('Failed to load FY consolidated data');
                });
        });
    </script>
    ...
    <script>
        waitForHighcharts(function () {
            ...
            // Load Marital Status Distribution
            function loadMaritalStatusDistribution() {
                fetch("{{ route('dashboard.maritalStatusDistribution') }}", {
                    method: 'GET',
