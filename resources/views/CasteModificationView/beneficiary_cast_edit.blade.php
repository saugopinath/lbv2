<x-layouts.app>
    ...
    <div x-data="{ selectedCaste: '{{ old('caste') ?? '' }}' }" 
    ...
    <form action="{{ route('beneficiary.updateCaste') }}" method="POST" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <x-form.select id="caste" name="caste" label="New Caste"
                    x-model="selectedCaste">
                    <option value="">-- Select New Caste --</option>
                    @foreach($castes as $key => $label)
                        <option value="{{ $key }}" {{ old('caste') == $key ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </x-form.select>
        ...
        <div class="flex justify-end">
            <x-button.indigo type="submit">Submit</x-button.indigo>
        </div>
    </form>
</div>
