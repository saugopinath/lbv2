...
class BaseManagementDetailsTable extends DataTableComponent
{
    ...
    public function builder(): Builder
    {
        $query = User::query()
            ->whereHas('RoleSchemeOfficeMappings.office', function ($q) {
                $q->where('is_active', 1)->where('is_base', true);
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Actions")
                ...
                ->html(),
                Column::make("Sync Permission")
                ->label(
                    fn($row) =>
                    view('coulmn_button.syncbutton', [
                        'wireClick' => " \$dispatch('syncpermission', { userId: {$row->id} })",
                        'tooltip'   => 'Update Permissions',
                    ])->render()
                )
                ->html(),
