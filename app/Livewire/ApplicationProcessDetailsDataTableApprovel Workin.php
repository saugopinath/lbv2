...
class ApplicationProcessDetailsDataTable extends DataTableComponent
{
    ...
    public function confirmBulkRevert($validated, WorkflowService $workflowService)
    {
        ...
        if ($this->revertrejectAction === 'revert') {
            ...
            // Get selected applications grouped by local body
            $applications = BeneficiaryPersonalDetail::whereIn('application_id', $ids)
                ->select(
                    'application_id',
                    'application_type',
                ...
                ->get()
                ->groupBy('created_by_local_body_code');

            foreach ($applications as $localBody => $apps) {

                $types = $apps->pluck('application_type')->toArray();
                $distCode = $apps->first()->created_by_dist_code;
                ...
                $check = \App\Helpers\SchemeCapacityHelper::checkBulk(
                    ...
                    [
                        ...
                        'created_by_local_body_code' => $localBody
                    ]
                );
                ...
                if (!$check['is_processed']) {
                    $this->dispatch('toastr', [
                        'type' => 'error',
                        'message' => "Capacity full for {$check['model']}! Remaining {$check['remaining_capacity']}."
                    ]);
                    return;
                }
            }
            ...
            foreach ($ids as $id) {
                ...
                try {
                    ...
                    BeneficiaryPersonalDetail::where('application_id', $id)
                        ->where($this->filter_condition)
                        ->update([
                            'next_level_role_id' => $this->nextLabelRoleId,
                            'is_clean' => 1,
                        ]);

                    $beneficiary_id = BeneficiaryPersonalDetail::where('application_id', $id)
