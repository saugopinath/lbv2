...
class ProcessApplicationTable extends DataTableComponent
{
    ...
    {
        if (empty($this->bulkActionType)) {
            $this->dispatch('notify', 'Please select an action');
            return;
        }

        $query = Application::whereIn('id', $this->selectedRows);

        if ($this->bulkActionType === 'approve') {
            $query->update(['status' => 2]); // Approved
        } elseif ($this->bulkActionType === 'reject') {
            $query->update([
                'status' => 3, // Rejected
                'reason' => $this->reason,
                'remark' => $this->remark,
            ]);
        } elseif ($this->bulkActionType === 'revert') {
            $query->update([
                'status' => 4, // Reverted
                'reason' => $this->reason,
                'remark' => $this->remark,
        ...
        // reset
        $this->reset(['selectedRows', 'bulkActionModal', 'bulkActionType', 'reason', 'remark']);

        $this->dispatch('notify', 'Bulk action performed successfully!');
    }
