...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        DB::connection($this->connection)->statement("
            CREATE MATERIALIZED VIEW IF NOT EXISTS pension.mv_scheme_status_summary_v1
            ...
            AS
             WITH step_map AS (
                     ...
                     SELECT schemes.id AS scheme_id,
                       ...
                       FROM schemes
                      WHERE schemes.is_active = 1 AND schemes.id NOT IN (20)
                    ), base AS (
                     SELECT b.scheme_id,
                        si.scheme_name,
                            CASE
        ...
        // ── 4. Set ownership ───────────────────────────────────────────────────
        DB::connection($this->connection)->statement(
            'ALTER TABLE IF EXISTS pension.mv_scheme_status_summary_v1 OWNER TO postgres'
        );

        // ── 5. Unique index (required for CONCURRENT refresh) ─────────────────────
        DB::connection($this->connection)->statement("
            CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_scheme_status_summary_v1_scheme_id
                ON pension.mv_scheme_status_summary_v1 USING btree (scheme_id)
                TABLESPACE pg_default
        ");
    }
