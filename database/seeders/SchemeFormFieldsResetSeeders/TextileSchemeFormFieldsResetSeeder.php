...
class TextileSchemeFormFieldsResetSeeder extends Seeder
{
    public function run(): void
    {
        $scheme = Scheme::where('short_name', 'weavers')
            ->orWhere('name', 'LIKE', '%TEXTILE%')
            ->first() ?? Scheme::find(7);

        if (!$scheme) {
            $this->command?->error('Textile Scheme not found!');
            return;
        }
