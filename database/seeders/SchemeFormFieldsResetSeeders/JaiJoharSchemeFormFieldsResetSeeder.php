...
class JaiJoharSchemeFormFieldsResetSeeder extends Seeder
{
    public function run(): void
    {
        $scheme = Scheme::where('short_name', 'johar')
            ->orWhere('name', 'LIKE', '%JOHAR%')
            ->first() ?? Scheme::find(1);

        if (!$scheme) {
            $this->command?->error('Jai Johar Scheme not found!');
            return;
        }
        ...
        DB::transaction(function () use ($schemeId) {
            ...
            $docs = [
                ...
                ['code' => '1614', 'is_required' => true, 'max_file_size' => '500KB', 'extension_type' => 'jpg,jpeg,png,pdf'], // Birth registration Certificate
        ...
        $this->command?->info("Jai Johar Scheme (Scheme ID {$schemeId}) form fields successfully reset and regenerated!");
    }
}
