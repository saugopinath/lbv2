...
class LPPRetainerSchemeFormFieldsResetSeeder extends Seeder
{
    public function run(): void
    {
        $scheme = Scheme::where('short_name', 'lokprasar_retainer')
            ->orWhere('name', 'LIKE', '%LPP RETAINER%')
            ->first() ?? Scheme::find(8);

        if (!$scheme) {
            $this->command?->error('LPP Retainer Scheme not found!');
            return;
        }
        ...
        DB::transaction(function () use ($schemeId) {
            ...
            // 5. Seed Form Fields for Tabs (LPP Retainer Tab 106: Ration Card, PAN, EPIC)
            $fieldsByTab = [
                101 => [
                    'application_date',
                    'application_type',
        ...
        $this->command?->info("LPP Retainer Scheme (Scheme ID {$schemeId}) form fields successfully reset and regenerated!");
    }
}
