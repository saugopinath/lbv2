...
class MSMESchemeFormFieldsResetSeeder extends Seeder
{
    public function run(): void
    {
        $scheme = Scheme::where('short_name', 'msme')
            ->orWhere('name', 'LIKE', '%MSME%')
            ->first() ?? Scheme::find(6);

        if (!$scheme) {
            $this->command?->error('MSME Scheme not found!');
            return;
        }
        ...
        DB::transaction(function () use ($schemeId) {
            ...
            // 6. Seed Attached Documents (Tab 104: Docs 1, 2, 3, 4, 8, 9)
            $docs = [
                ...
                ['code' => '166', 'is_required' => false, 'max_file_size' => '500KB', 'extension_type' => 'jpg,jpeg,png,pdf'],
            ];
        ...
        $this->command?->info("MSME Scheme (Scheme ID {$schemeId}) form fields successfully reset and regenerated!");
    }
}
