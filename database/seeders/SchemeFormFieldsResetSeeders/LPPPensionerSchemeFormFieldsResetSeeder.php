...
class LPPPensionerSchemeFormFieldsResetSeeder extends Seeder
{
    public function run(): void
    {
        $scheme = Scheme::where('short_name', 'lokprasar_pensioner')
            ->orWhere('name', 'LIKE', '%LPP PENSIONER%')
            ->first() ?? Scheme::find(9);

        if (!$scheme) {
            $this->command?->error('LPP Pensioner Scheme not found!');
            return;
        }
        ...
        DB::transaction(function () use ($schemeId) {
            ...
            // 5. Seed Form Fields for Tabs (LPP Pensioner Tab 106: Ration Card, PAN, EPIC)
            $fieldsByTab = [
                101 => [
                    'application_date',
                    'application_type',
        ...
        $this->command?->info("LPP Pensioner Scheme (Scheme ID {$schemeId}) form fields successfully reset and regenerated!");
    }
}
